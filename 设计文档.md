...
| v1.1 | 基于对战分析(PK 580611)优化策略 | 2026-09-13 |
...
### 1.4 基于对战分析的策略优化（v1.1）

**对战数据**: PK 580611 (trailblazer vs Agentic麻辣烫, 0:3完败)

**发现的关键问题**:

1. **P1 - 经济冻结**: 开局75金币在6回合内全部用于建造3座塔，第7回合起金币恒为0，无任何收入循环
2. **P2 - 围墙缺失**: 到第13回合围墙仍为0段，`STONE_BATCH=6`导致工人需攒满6块石头才建墙，启动太慢
3. **P3 - 任务断链**: 开拓者第11回合才接任务，且缺少`submit_answer_command`调用，160分+160金币全部泡汤
4. **P4 - 塔位集中**: 3座塔全部挤在基地左侧同一列(29,8-10)，右侧/下侧无火力覆盖
5. **P5 - 夜晚空转**: 白天角色远离基地采集，夜晚武器操控者不能及时就位，火力空转
6. **P6 - LLM无效**: LLM建议从未被决策逻辑读取，`buy_command`/`use_command`等指令零调用

**已实施的优化方案**:

| 优化项 | 改动模块 | 具体措施 | 预期效果 |
|-------|---------|---------|---------|
| **S1: 围墙提前** | brain.py | `STONE_BATCH: 6→3`，工人有1-2块石头就开建 | 第5-6回合立起第一段墙 |
| **S2: 任务完成** | brain.py, protocol.py | 新增`_task_answer()`解析沙盒输出，调用`submit_answer_command()` | 直接兑换160分+160金币 |
| **S3: 塔位分散** | brain.py | `_calc_tower_sites()`按四方位(上/左/下/右)分组，每方位取1座 | 全方位火力覆盖 |
| **S4: 经济循环** | brain.py | 围墙建完后：金币≥100买升级券→升级武器；卖石头换金币 | 金币持续滚动 |
| **S5: 黄昏回防** | brain.py | 距天黑≤5回合时，角色强制回到武器旁 | 首夜三塔满火力 |
| **S6: 威胁优先** | brain.py | 攻击目标按`ROBOT_THREAT`分级(BOSS>大型>中型>小型) | 优先清除高威胁 |

**关键代码改动**:

```python
# 策略常量优化
STONE_BATCH = 3  # 原值6，降低到3让围墙更早开工
DUSK_ROUNDS = 5  # 天黑前5回合开始回防
WEAPON_UPGRADE_VOUCHER = "WeaponUpgradeVoucher1"
UPGRADE_GOLD = 100
...
# 新增函数
def _upgrade_weapon_with_gold(...):  # 武器升级逻辑
def _fall_back_to_weapons(...):      # 黄昏回防逻辑
def _task_answer(...):               # 任务答案解析
def _sandbox_command(...):           # 沙盒命令生成
def sandbox_command(payload):        # 暴露给server.py的接口
```
...
STONE_BATCH = 3  # 工人采集石头的批次大小（v1.1优化：6→3，让围墙更早开工）
WALL_BUILD_PRIORITY = 1000  # 围墙建造优先级
SELL_BATCH = 10  # 卖给小贩的石头批次大小
DUSK_ROUNDS = 5  # 天黑前提前回防的回合数（v1.1新增）
WEAPON_UPGRADE_VOUCHER = "WeaponUpgradeVoucher1"  # 武器升级券（v1.1新增）
UPGRADE_GOLD = 100  # 购买一张武器升级券所需金币（v1.1新增）

# 机器人威胁等级（v1.1新增：数值越大越优先处理）
ROBOT_THREAT = {
    "bossRobot": 3,
    "largeRobot": 2,
    "middleRobot": 1,
...
def decide(payload: dict[str, Any]) -> tuple[dict[str, dict[str, Any]], str]:
    ...
    主决策函数
    输入: 判题系统发来的游戏状态
    输出: (角色ID字符串: 指令字典, LLM提示词)
    
    v1.1变更: 返回值改为元组，新增LLM提示词用于策略咨询
    """
    ...
    # v1.1新增：生成LLM策略咨询prompt（每天第一个回合）
...
def _worker_day_logic(
    ...
    # v1.1新增：围墙已建完，把富余资源换成战力
    if not walls_missing:
        # 优先：金币换武器升级券并升级武器（level1 -> level2）
...
def _pioneer_day_logic(
    ...
    优先级（v1.1优化）: 
        维持进行中的任务（含提交答案） > 前往任务点领取任务 > 跟随武器塔
    ...
    if turn.phase_task:
        ...
        if task_pos is not None:
            if distance(pioneer.pos, task_pos) > 1:
                ...
                return
            # 已在任务点旁，检查是否有答案可提交
            answer = _task_answer(turn, pioneer)
            if answer:
                commands[pioneer.unit_id] = submit_answer_command(answer)
            return
    
    # 2. 无任务时: 寻找可领取的任务点（isValid=true且未冷却）
    task_point = _nearest_valid_task_point(turn, pioneer.pos)
    if task_point is not None:
        if distance(pioneer.pos, task_point) <= 1:
            commands[pioneer.unit_id] = accept_task_command()
            return
        step = _step_toward(turn, pioneer, task_point, claimed)
        if step is not None:
            commands[pioneer.unit_id] = move_command(step)
            return
    
    # 3. 无可做任务: 跟随武器塔，但不阻挡围墙建造点
...
def _find_attack_target(turn: Turn, weapon: Unit) -> Pos | None:
    """为武器寻找攻击目标（优先机器人，按威胁等级排序）
    
    v1.1优化：攻击目标按威胁分级（BOSS > 大型 > 中型 > 小型），同级别取最近
    """
    ...
    if targets_in_range:
        # v1.1优化：按威胁等级排序，同级别按距离排序
        def threat_key(robot):
            threat_level = ROBOT_THREAT.get(robot.robot_type, 0)
            dist = distance(weapon.pos, robot.pos)
            return (-threat_level, dist, robot.robot_id)
        
        nearest = min(targets_in_range, key=threat_key)
        return nearest.pos
...
def _rounds_to_night(turn: Turn) -> int:
    ...
    if round_in_day < DAY_ROUNDS:
        return DAY_ROUNDS - round_in_day
    return 0  # 已经是夜晚
...
def _fall_back_to_weapons(
    ...
    # 找最近的武器
    nearest_weapon = min(weapons, key=lambda w: distance(unit.pos, w.pos))
    
    # 已在武器旁，停止
    if distance(unit.pos, nearest_weapon.pos) <= 1:
        return
    
    # 向武器移动
    step = _step_toward(turn, unit, nearest_weapon.pos, claimed)
    if step is not None:
        commands[unit.unit_id] = move_command(step)
...
def _upgrade_weapon_with_gold(
    ...
    # 找到所有level1的武器
    weapons_lv1 = [w for w in turn.weapons() if w.level == 1]
    if not weapons_lv1:
        return False  # 没有需要升级的武器
    
    # 检查背包里是否已有升级券
    has_voucher = WEAPON_UPGRADE_VOUCHER in worker.backpack
    
    if not has_voucher:
        # 去武器商店买升级券
        shop_pos = turn.find_zone(WEAPON_SHOP)
        if shop_pos is None:
            return False
        
        if distance(worker.pos, shop_pos) <= 1:
            # 在商店旁，购买升级券
            commands[worker.unit_id] = buy_command(WEAPON_UPGRADE_VOUCHER, 1)
            return True
        else:
            # 向商店移动
        ...
        # 手里有升级券，去升级最近的level1武器
        nearest_weapon = min(weapons_lv1, key=lambda w: distance(worker.pos, w.pos))
        
        if distance(worker.pos, nearest_weapon.pos) <= 1:
            # 在武器旁，使用升级券
            commands[worker.unit_id] = use_command(WEAPON_UPGRADE_VOUCHER, nearest_weapon.pos)
            return True
        else:
            # 向武器移动
            step = _step_toward(turn, worker, nearest_weapon.pos, claimed)
            if step is not None:
                commands[worker.unit_id] = move_command(step)
                return True
...
def _trade_logic(
    ...
    # 石头不足一批，不卖
    if stones < SELL_BATCH:
        return
    
    # 找小贩位置
    vendor_pos = turn.find_zone(VENDOR)
    if vendor_pos is None:
        return
    
    if distance(worker.pos, vendor_pos) <= 1:
        # 在小贩旁，卖石头
        commands[worker.unit_id] = sell_command(WALL_MATERIAL, SELL_BATCH)
    else:
        # 向小贩移动
        step = _step_toward(turn, worker, vendor_pos, claimed)
        if step is not None:
            commands[worker.unit_id] = move_command(step)


def _nearest_task_position(turn: Turn, pos: Pos) -> Pos | None:
    """找到最近的己方任务点位置"""
    team_type = turn.team_type
    task_types = _TASK_POINTS_BY_TEAM.get(team_type, ())
    
    tasks = []
    for task_type in task_types:
        task_pos = turn.find_zone(task_type)
        if task_pos:
            tasks.append(task_pos)
    
    if not tasks:
        return None
    
    return min(tasks, key=lambda t: distance(pos, t))


def _nearest_valid_task_point(turn: Turn, pos: Pos) -> Pos | None:
    """找到最近的可领取任务点（isValid=true且未冷却）"""
    team_type = turn.team_type
    task_types = _TASK_POINTS_BY_TEAM.get(team_type, ())
    ...
    for task_type in task_types:
        task_pos = turn.find_zone(task_type)
        if task_pos and turn.is_task_valid(task_type):
            valid_tasks.append(task_pos)
    
    if not valid_tasks:
        return None
    
    return min(valid_tasks, key=lambda t: distance(pos, t))


def _task_answer(turn: Turn, pioneer: Unit) -> str:
    """从沙盒输出中提取任务答案
    
    任务流程：
        1. acceptTask后，turn.phase_task出现任务描述（如"请阅读task_1_beijing.md"）
        2. _sandbox_command()提取文件名并生成shell命令（如"cat task_1_beijing.md"）
        3. 下一回合，turn.last_cmd_result包含沙盒输出
        4. 本函数从沙盒输出中解析答案
    
    返回:
        任务答案字符串；如果沙盒输出尚未就绪或解析失败则返回空字符串
    """
    ...
    # 沙盒输出格式：[exitCode:N]\n任务标识\n实际内容
    # 任务标识格式：[TASK]<任务描述的前20字符>
    task_token = _task_token(turn.phase_task)
    if task_token not in result:
        return ""  # 输出不属于当前任务
    
    # 提取exitCode之后、任务标识之后的内容作为答案
    lines = result.split("\n", 2)
    if len(lines) < 3:
        return ""
    
    # lines[0]: [exitCode:0]
    # lines[1]: [TASK]请阅读task_1_beij
    # lines[2]: 实际文件内容（任务答案）
...
def _task_token(phase_task: str) -> str:
    """生成任务标识符（用于匹配沙盒输出）"""
    return f"{TASK_MARKER}{phase_task[:20]}"
...
def _sandbox_command(turn: Turn) -> str:
    ...
    下一回合通过请求的 lastCmdResult 字段返回，再由 _task_answer 解析成
    submitAnswer 的答案。
    ...
    # 已有答案，不再重复执行
    if _task_answer(turn, turn.pioneers()[0] if turn.pioneers() else None):
        return ""
    
    # 从任务描述中提取文件名（如"请阅读task_1_beijing.md"）
    match = TASK_FILE_PATTERN.search(turn.phase_task)
    if not match:
        return ""
    
    filename = match.group(0)
    
    # 生成shell命令：读取文件内容，并加上任务标识前缀
    task_token = _task_token(turn.phase_task)
    return f'echo "{task_token}" && cat {filename}'


def _generate_strategy_prompt(turn: Turn, payload: dict) -> str:
    """生成LLM策略咨询prompt（每天第一个回合调用一次）
    
    v1.1新增：让LLM根据当前游戏状态提供策略建议
    
    返回:
        LLM提示词字符串；非每日首回合或LLM功能关闭时返回空字符串
    """
    # 只在每天第一个回合且LLM开启时生成prompt
    round_in_day = (turn.round_no - 1) % ROUNDS_PER_DAY
    if round_in_day != 0 or not LLM_PROMPT_ENABLED:
        return ""
    
    # 构造prompt，包含当前状态摘要
    day_no = (turn.round_no - 1) // ROUNDS_PER_DAY + 1
    prompt = f"""第{day_no}天开始。当前状态：
- 金币：{turn.gold}
- 我方单位：工人{len(turn.workers())}、开拓者{len(turn.pioneers())}
- 已建武器：{len(turn.weapons())}座
- 已建围墙：{len(turn.walls())}段
- 累计积分：{turn.total_score}
...
def _calc_tower_sites(turn: Turn) -> tuple[Pos, ...]:
    """计算武器塔建造位置（基地周围一圈，按方位分散）
    
    v1.1优化：将候选位置按四方位（上/左/下/右）分组，每个方位取最优1座，
             保证三座塔不会全部挤在基地同一侧，实现全方位火力覆盖
    """
    station = turn.station()
    if station is None:
        return ()
    ...
    # v1.1新增：按方位分组（相对于基地中心）
    center_x = sum(p.x for p in footprint) // len(footprint)
    center_y = sum(p.y for p in footprint) // len(footprint)
    
    def classify_side(pos: Pos) -> str:
        """判断位置在基地的哪一侧"""
        dx = pos.x - center_x
        dy = pos.y - center_y
        if abs(dy) > abs(dx):
    ...
    for side in ("up", "left", "down", "right"):
        if by_side[side]:
            best = min(by_side[side], key=lambda p: (
                _footprint_distance(p, footprint), p.x, p.y
            ))
            sites.append(best)
    
    # 返回前3个（保证至少覆盖3个不同方位）
    return tuple(sites[:3])
...
根据issue #8的实际对战分析需求，自动化系统已扩展支持对战日志分析功能。

#### 11.8.1 对战分析Issue格式

对战系统会自动创建格式化的Issue，包含：
...
**结果**: teamAName=Agentic麻辣烫; teamBName=trailblazer; scoreA=3; scoreB=0
...
3. **代码修改**：
   - 将优化建议的"涉及文件"和"做法"提取成Claude提示词
   - 调用Claude Code执行修改
   - 运行测试验证修改正确性

4. **PR创建**：
   - 分支命名：`auto-fix/issue-8-20260913-203502`
   - 提交消息：`[Auto-Fix] #8: [优化建议标题] (回合数)`
   - PR描述：包含原Issue链接、修改文件列表、预期效果

#### 11.8.3 配置扩展

在`automation_config.yaml`中添加：

```yaml
battle_analysis:
  enabled: true
  auto_apply: "S1"  # 自动应用S1建议，或"all"应用所有
  run_tests: true   # 修改后自动运行测试
  test_command: "cd CoreGeek && pytest tests/"
  max_suggestions: 3  # 单次最多应用的建议数
```
...
**应用优化**：
- S1：围墙提前介入（STONE_BATCH从6降到3）
- S2：补全任务完成闭环（submitAnswer调用）
- S3：塔位按方位分散（四方位算法）

**自动化执行**：
1. 解析issue内容，提取S1-S5建议
2. 生成Claude提示词：
   ```
   根据对战分析报告issue #8的S1建议，修改以下文件：
   - CoreGeek/src/agent/brain.py
   
   修改内容：
   1. STONE_BATCH由6降到2-3
   2. 修改L174采集条件为`stones < min(STONE_BATCH, 2)`
   3. 开局前3回合建完2座塔后暂停，先采石建墙
   
   目标：第5-6回合就能立起第一段围墙，第1天白天结束前形成闭环防线。
   ```
3. Claude执行修改
4. 运行`pytest tests/test_brain.py`验证
5. 创建PR: https://github.com/ScreaMA/Competition10/pull/9
...
日志示例：
```
2026-09-13 20:35:02 | INFO | Detected new issue #8: 对战分析报告 (PK 580611)
2026-09-13 20:35:02 | INFO | Creating branch: auto-fix/issue-8-20260913-203502
2026-09-13 20:35:05 | INFO | Executing Claude with prompt from S1-S3
2026-09-13 20:37:21 | INFO | Claude execution completed
2026-09-13 20:37:22 | INFO | Running tests: pytest tests/test_brain.py
2026-09-13 20:37:45 | INFO | Tests passed: 28/28
2026-09-13 20:37:46 | INFO | Pushing to GitHub
2026-09-13 20:37:48 | INFO | Created PR #9: https://github.com/ScreaMA/Competition10/pull/9
2026-09-13 20:37:49 | INFO | Commented on issue #8
```

### 11.10 扩展功能

**未来可添加**：
1. **Webhook模式**：GitHub Issue创建时立即触发（需公网IP或ngrok）
2. **多策略测试**：对同一Issue创建多个分支，分别应用不同优化建议
3. **A/B对战测试**：修改后自动提交对战，比较优化效果
4. **性能监控**：跟踪历史对战数据，生成趋势图表
5. **自动回滚**：如果新版本对战表现下降，自动revert
