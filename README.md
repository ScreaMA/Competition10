...
| 角色 | 优先级 | 行为 |
|-----|-------|------|
| 工人 | 1 | 金币≥25时建造3座武器塔（加特林、电磁狙击炮、火箭发射台），**三塔分散在基地四方位（上/左/下/右）**；**塔位这一回合走不通时继续去采集/建墙，不再空手过回合** |
...
| 工人 | 5 | **围墙建完后，金币≥100时买升级券升级武器（level1→level2）**；分工里的第2名工人从第2优先级起就负责卖矿换金币与买券，**不必等围墙圈建完** |
...
- ✅ **围墙先封敌方来路**：`_calc_wall_order()` 复用塔位那套方位判定（`_enemy_sides`），
  ...
  看不到敌方单位时顺序与改造前完全一致
- ✅ **防守方围墙下限**：防守方每天至少保证铺好 `DEFENDER_WALL_QUOTA`（=1）段围墙
  （`_wall_target`），LLM 计划把墙压到 0 段也先留出石材施工——复盘里防守方整天
  0 段围墙、正面毫无阻挡，机器人直接贴脸打基地；进攻方不受影响，仍完全按计划走
  （地图上没有任何石材来源时下限自动失效，免得把经济线扣住、矿石卖不掉）
