...
**任务全量信息一律走 INFO、不精简**：`execute_cmd` 打的是完整命令（含脚本全文），
沙盒原文、提交的答案也一样。判题器采集的是进程 stdout，**那是对局结束后唯一的
取日志手段**——任何"压到 DEBUG"的省体积做法都等于把复盘证据丢掉。

比 V1 多出来的是**逐回合增量**（击杀/掉血/建筑损失/空转）、**任务链路事件**
（start/step/submit/abandon/skill）与**金币停滞自动告警**——V1 报告里大量
"日志未覆盖"的结论就是因为日志只有状态快照、没有增量。

**先看 [战术参考/策略与坑位总纲.md](./战术参考/策略与坑位总纲.md)** —— 打的时候怎么想、
哪里踩过雷（每条坑带实测回合号）、可调常量速查、改完代码跑什么。
